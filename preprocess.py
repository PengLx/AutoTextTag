...
def convert_examples_to_features(examples, max_seq_len, bert_dir, label2id):
    tokenizer = BertTokenizer.from_pretrained('bert-base-cased', cache_dir='./model')
    features = []
    callback_info = []
...
def out(file_path, args, label2id, mode, inference_list=[]):
    if mode == "infer":
        data = pd.DataFrame({"sentence": inference_list})
        data["labels"] = "[]"
    else:
        data = pd.read_csv(file_path, sep='ㅇ')
    examples = []
